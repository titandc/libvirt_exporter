...
type LibvirtExporter struct {
	...
	libvirtDomainNumbers *prometheus.Desc
...
func (e *LibvirtExporter) CollectFromLibvirt(ch chan<- prometheus.Metric) error {
	...
	domainNumber := len(domainIds)
	ch <- prometheus.MustNewConstMetric(
		e.libvirtDomainNumbers,
		prometheus.GaugeValue,
...
func (e *LibvirtExporter) CollectDomain(ch chan<- prometheus.Metric, domain *libvirt.Domain) error {
	...
	memUnused := uint64(0)
	memMax := uint64(0)
	memStat, err := domain.MemoryStats(11, 0)
	for _, buf := range memStat {
		if buf.Tag == int32(libvirt.DOMAIN_MEMORY_STAT_UNUSED) {
			memUnused = buf.Val
		} else if buf.Tag == int32(libvirt.DOMAIN_MEMORY_STAT_ACTUAL_BALLOON) {
			memMax = buf.Val
		}
	}
	memUsed := memMax - memUnused
...
func main() {
	var (
		...
		listenAddress             = app.Flag("web.listen-address", "Address to listen on for web interface and telemetry.").Default(":9101").String()
